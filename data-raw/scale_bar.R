...
# Ewen Gallic scale bar functions: 
# 
...
create_scale_bar <- function(lon,lat,distance_lon,distance_lat,distance_legend, dist_units = "km"){
  ...
  if( dist_units == 'm') { 
    distance_lon_lab <- distance_lon
    distance_lon <- distance_lon/1000 
    distance_lat <- distance_lat/1000
    distance_legend <- distance_legend/1000
    dist_units <- 'km' 
    dist_label <- 'm' 
  }else{ 
    distance_lon_lab <- distance_lon
    dist_label <- dist_units
  } 
  ...
  legend <- data.frame(cbind(legend, text = c(0, distance_lon_lab, distance_lon_lab*2)), stringsAsFactors = FALSE, row.names = NULL)
...
scale_bar <- function(lon, lat, distance_lon, distance_lat, distance_legend, dist_units = "km", rec_fill = "white", rec_colour = "black", rec2_fill = "black", rec2_colour = "black", legend_colour = "black", legend_size = 3, orientation = TRUE, arrow_length = 500, arrow_distance = 300, arrow_north_size = 6){
  ...
  the_scale_bar <- create_scale_bar(lon = lon, lat = lat, distance_lon = distance_lon, distance_lat = distance_lat, distance_legend = distance_legend, dist_units = dist_units)
  # First rectangle
  ...
  if(dist_label == 'm'){ 
    the_scale_bar$legend[, 'text'] <- the_scale_bar$legend[, 'text']*1000 
  }
  
  # Legend
  scale_bar_legend <- annotate("text", 
                               label = paste(the_scale_bar$legend[,"text"], 
                                             dist_label, sep=""), x = the_scale_bar$legend[,"long"], y = the_scale_bar$legend[,"lat"], size = legend_size, colour = legend_colour)
  
  res <- list(rectangle1, rectangle2, scale_bar_legend)
  
  if(orientation){# Add an arrow pointing North
    coords_arrow <- create_orientation_arrow(scale_bar = the_scale_bar, length = arrow_length, distance = arrow_distance, dist_units = dist_units)
